...
function Landingpage(props) {
  ...
  return (
    <div
      data-scroll
      data-scroll-speed={props.scroll}
      className='w-full lg:h-screen md:h-full bg-zinc-900 pt-1 text-white'>
              ...
              <div className='w-fit flex items-end overflow-hidden'>
                {index === 1 && (
                  <motion.div
                    ...
                    <img
                      className='mr-[.2vw] lg:mb-2 sm:mb-1  w-[8vw] rounded-lg h-[5vw] pr-1'
                      src={props.url}
                      alt=''
                    />
                  </motion.div>
      ...
      <div className='px-8 py-5 border-t-[1px] border-zinc-800 mt-32 flex justify-between content-baseline'>
        {[props.subtitle, props.description].map((text, index) => (
          <div className='masker' key={index}>
            <p
              key={index}
              className='lg:text-lg tracking-tight leading-none capitalize'>
              {text}
            </p>
          </div>
        ))}
